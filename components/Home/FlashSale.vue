<template>
  <div class="container mx-auto bg-white mt-[4rem]">
    ...
    <div class="grid gap-2 sm:grid-cols-2 md:grid-cols-4 mx-6">
      <div v-for=" item  in flash_sale?.data?.products" :key=" item.id " class="crad p-2 flex flex-col">
        <div class="relative rounded-[1rem] overflow-hidden" v-if=" item.flash_sale_product ">
          <img :src=" item.flash_sale_product?.product_details[0]?.images[0]?.url " alt="card-image"
        ...
        <div class="pt-[5px]">
          <NuxtLink :to=" localePath(`/products/${item.flash_sale_product.id }`)">
          ...
          <div class="mt-1">
            ...
            <span class="text-gray px-2 text-[12px] line-through">{{ item.flash_sale_product.product_details[0].currency
            }}
              {{ item.flash_sale_product.product_details[0].price }}</span>
          </div>
        </div>
        <div class="flex justify-between items-center mt-auto">
          <div class="block bg-yellow h-auto font-[600] rounded text-[12px] px-[5px] w-9/12">
            Out of Stock
    ...
    <div class="grid gap-5 md:grid-cols-2">
      <div class="flash-sale relative p-7">
        <div>
          <img class="w-100" src="../../assets/images/flash2.png" alt="jewellery" />
        </div>
        <div class="text-center absolute bottom-20 left-0 right-0 m-[0 auto]">
          <h4 class="font-[700] text-[4rem] text-white italic">Get Jeweled</h4>
          <NuxtLink class="font-[500] text-[1rem] uppercase underline text-white" :to="localePath('/')">show your jewelry
            now</NuxtLink>
        </div>
      </div>
      <div class="flash-sale relative p-7">
        <div>
          <img class="w-100" src="../../assets/images/flash3.png" alt="jewellery" />
        </div>
        <div class="text-center absolute bottom-20 left-0 right-0 m-[0 auto]">
          <h4 class="font-[700] text-[4rem] text-white italic">
            End Of Summer
          </h4>
          <NuxtLink class="font-[500] text-[1rem] uppercase underline text-white" :to="localePath('/')">show from here
          </NuxtLink>
        </div>
      </div>
    </div>
